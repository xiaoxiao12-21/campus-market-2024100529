<template>
  <DetailBack>
    ...
    <ErrorState v-else-if="error" message="商品详情加载失败" show-retry @retry="loadData" />
      ...
      <div class="detail-body">
        ...
        <div class="info-grid">
          <div class="info-item">
            ...
            <span class="info-tag">{{ item.category }}</span>
          </div>
          <div class="info-item">
...
const isFavorited = computed(() => {
  ...
  return favoriteStore.isFavorite('trade', Number(item.value.id))
})

const statusText = computed(() => {
  if (!item.value) return ''
  return item.value.status === 'open' ? '在售' : '已售出'
})
...
function toggleFavorite() {
  ...
  if (isFavorited.value) {
    favoriteStore.removeFavorite('trade', id)
